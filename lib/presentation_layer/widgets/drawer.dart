...
class MyDrawer extends StatelessWidget {
  ...
  Widget buildDrawerHeader(context){
    return Column(
      children: [
        ...
        Text('Ahmed Abdraboo',style: TextStyle(fontSize: 20,fontWeight: FontWeight.bold),),
        const SizedBox(height: 5,),
        BlocProvider<PhoneAuthCubit>(
            create: (context)=>phoneAuthCubit,
          child:Text('${phoneAuthCubit.getLogedInUser().phoneNumber}',style: TextStyle(fontSize: 18,fontWeight: FontWeight.bold)),
        ),
      ],
    );
  }
  ...
  void _launchURL(String url)async{
    await canLaunch(url)? await launch(url): throw 'couldnt launch $url';
  }
  ...
  Widget buildSocialMediaIcons(){
    return Padding(
      ...
      child: Row(
        children: [
          buildIcon(FontAwesomeIcons.facebook, 'url'),
          const SizedBox(width: 15,),
          buildIcon(FontAwesomeIcons.youtube, 'url'),
          const SizedBox(width: 20,),
          buildIcon(FontAwesomeIcons.telegram, 'url'),
        ],
      ),
    );
  }
